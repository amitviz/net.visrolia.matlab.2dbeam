    ...
    case 1
        ...
        if (max(varargin{1}) - min(varargin{1})) ~= 0
            ...
            elseif scalefactor == 0
                % this happens for a 1D problem
                scalefactor = 0.5*max(max(nodes) - min(nodes))/(10^ceil(log10(max(abs(u(ud))))));
            end
...
disp(' e  ex_max    M_max     Ry1       M1        Ry2       M2      ');
